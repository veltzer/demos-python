...
if pid == 0:
    ...
    if os.WIFEXITED(ret):
        print(
            'process was signaled and signal was [{0}]'.format(os.WEXITSTATUS(ret)))
    if os.WIFSTOPPED(ret):
        print(
            'process was signaled and signal was [{0}]'.format(os.WSTOPSIG(ret)))
    if os.WIFSIGNALED(ret):
        print(
            'process was signaled and signal was [{0}]'.format(os.WTERMSIG(ret)))
